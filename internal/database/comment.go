...
func (d *Database) UpdateComment(ctx context.Context, id string, cmt comment.Comment) (comment.Comment, error) {
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("failed to update comment in database: %w", err)
	}

	if err := rows.Close(); err != nil {
		return comment.Comment{}, fmt.Errorf("failed to close database rows: %w", err)
	}
...
/*
	We will be talking to Database in this file
*/
